...
public class DeviceEditPart extends EditPartWithListener implements NodeEditPart{

	private DeviceDirectEditManager directManager = null;
	...
	private void performDirectEdit(){
		if(directManager == null){
			directManager = new DeviceDirectEditManager(
					this, 
	...
	public void propertyChange(PropertyChangeEvent event){
		if(event.getPropertyName().equals(Device.P_CONSTRAINT))
			this.refreshVisuals();
		else if(event.getPropertyName().equals(Device.P_TEXT)){
			Label label = (Label)this.getFigure();
			label.setText(Device.type + "\n" + (String)event.getNewValue());
		}
		else if(event.getPropertyName().equals(Device.P_SOURCE_LOCATEIN_CONNECTION))
			refreshSourceConnections();
		else if(event.getPropertyName().equals(Device.P_TARGET_LOCATEIN_CONNECTION))
			refreshTargetConnections();
		else if(event.getPropertyName().equals(Device.P_TARGET_BELONGTO_CONNECTION))
			refreshTargetConnections();

	}
	
	protected List<?> getModelSourceConnections(){
		return ((Device)this.getModel()).getSourceConnections();
	}
	
	protected List<?> getModelTargetConnections(){
		return ((Device)this.getModel()).getTargetConnections();
	}
	
	protected void refreshVisuals(){
		Rectangle constraint = ((Device)getModel()).getConstraint();
